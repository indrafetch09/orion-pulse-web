...
export const getSolution = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!solution) {
      return res
        .status(404)
        .json({ success: false, message: "AI Solution not found" });
    }
    ...
    if (!logObj) {
      return res
        .status(404)
        .json({ success: false, message: "Associated port log not found" });
    }
    ...
    if (!portObj) {
      return res
        .status(404)
        .json({ success: false, message: "Associated port config not found" });
    }
    ...
    if (!serverObj) {
      return res.status(401).json({
        success: false,
        message: "Unauthorized access to this AI solution",
      });
    }
