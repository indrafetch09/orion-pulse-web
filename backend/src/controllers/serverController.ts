...
export const getAll = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const servers = await Server.find({ userId: req.user.id }).sort({
      createdAt: -1,
    });
...
export const create = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!name || !hostname) {
      return res
        .status(400)
        .json({ success: false, message: "Name and hostname are required" });
    }
...
export const deleteServer = async (
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      message:
        "Server and all related monitored resources deleted successfully",
    });
  } catch (error: unknown) {
    return res.status(500).json({ success: false, message: error });
  }
