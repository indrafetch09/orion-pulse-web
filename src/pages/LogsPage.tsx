...
const mockLogs: PortLog[] = Array.from({ length: 20 }, (_, i) => ({
  ...
  status: (['open', 'open', 'closed', 'open', 'filtered', 'open'] as const)[i % 6],
...
export default function LogsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filters */}
      <div className="flex flex-wrap gap-4 animate-fade-in" style={{ animationDelay: '50ms' }}>
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Port</label>
          <div className="flex gap-1.5">
            {portFilters.map((f) => (
              <button
                key={f}
                onClick={() => { setPortFilter(f); setPage(1) }}
                className={cn(
                    ...
                    return (
                      <tr
                        ...
                        className={cn(
                          ...
                          (log.status === 'closed' || log.status === 'filtered') &&
                            'border-l-2',
      ...
      {filteredLogs.length > ITEMS_PER_PAGE && (
        <div className="flex items-center justify-between animate-fade-in">
          <p className="text-xs text-muted-foreground">
            Showing {(page - 1) * ITEMS_PER_PAGE + 1}–{Math.min(page * ITEMS_PER_PAGE, filteredLogs.length)} of {filteredLogs.length} logs
          </p>
      ...
      <Dialog open={clearDialogOpen} onOpenChange={setClearDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This action cannot be undone. All scan logs will be permanently deleted.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => setClearDialogOpen(false)}>
