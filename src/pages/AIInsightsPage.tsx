...
const mockSolutions: AISolution[] = [
  {
    ...
    portNumber: 5432,
    analysis:
      'Port 5432 (PostgreSQL) has been consistently returning "Connection refused" errors for the past 15 minutes. This typically indicates the PostgreSQL service is not running or has crashed.',
    solution:
  ...
  },
  {
    ...
    portNumber: 3306,
    analysis:
      'Port 3306 (MySQL) is showing "filtered" status which suggests a firewall rule is blocking incoming connections to the MySQL service.',
    solution:
      "1. Check firewall rules: sudo iptables -L -n | grep 3306\n2. Allow MySQL port: sudo ufw allow 3306/tcp\n3. Verify MySQL is listening: sudo ss -tlnp | grep 3306\n4. Check MySQL bind-address in /etc/mysql/mysql.conf.d/mysqld.cnf",
    confidence: 87,
  ...
  },
  {
    ...
    portNumber: 8080,
    analysis:
      "Port 8080 (Express API) experienced intermittent timeouts. Response times spiked from average 35ms to over 500ms, indicating possible resource exhaustion or memory leak.",
    solution:
      "1. Monitor Node.js memory usage: node --inspect your-app.js\n2. Check for memory leaks using clinic.js: npx clinic doctor -- node server.js\n3. Review recent deployments for performance regressions\n4. Consider implementing connection pooling if not already in place",
    confidence: 78,
  ...
  },
  {
    ...
    portNumber: 3000,
    analysis:
      "Port 3000 (React Dev Server) went offline briefly. This is common during hot module replacement (HMR) recompilation and is expected behavior during development.",
    solution:
      "No action required. This is normal behavior for Vite/React development servers during code changes. If the port remains down for extended periods:\n1. Check terminal for compilation errors\n2. Restart dev server: npm run dev\n3. Clear node_modules/.vite cache",
    confidence: 95,
    ...
    <div className="space-y-6">
      ...
      {filteredSolutions.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground animate-fade-in">
          ...
          <p className="text-sm">
            AI insights will appear here when port issues are detected.
          </p>
        </div>
      ) : (
              ...
              <CardHeader className="pb-3">
                <div className="flex flex-wrap items-center gap-3">
                  <Badge variant="default" className="font-mono">
                    <Database className="mr-1 h-3 w-3" />:{sol.portNumber}
                  </Badge>
